    ...
    return (
        <nav className={`navbar ${isSticky ? 'sticky' : ''}`}>
            <div className="navbar-container">
                <button className={`navbar-hamburger`} onClick={handleHamburgerClick} aria-label="Toggle menu">
                ...
                <div className="navbar-left">
                    ...
                    {showMobileMenu && (
                        <ul className="nav-menu-mobile">
                            <li className="nav-item">
                                <NavLink to="/home" className="nav-links" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Home
                                </NavLink>
                            </li>
                            <li className="nav-item">
                                <NavLink to="/pricing" className="nav-links" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Pricing
                                </NavLink>
                            </li>
                            <li className="nav-item">
                                <NavLink to="/blog" className="nav-links" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Blog
                                </NavLink>
                            </li>
                            <li className="nav-item">
                                <NavLink to="/wall-of-love" className="nav-links" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Wall of Love
                                </NavLink>
                            </li>
                            <li className="nav-item dropdown">
                                ...
                                <ul className={`dropdown-menu${isMobileDropdownOpen ? ' open' : ''}`}>
                                    <li>
                                        <NavLink to="/support" className="dropdown-link" onClick={() => { handleLinkClick(); setShowMobileMenu(false); setIsMobileDropdownOpen(false); }}>
                                            Support
                                        </NavLink>
                                    </li>
                                    <li>
                                        <NavLink to="/404" className="dropdown-link" onClick={() => { handleLinkClick(); setShowMobileMenu(false); setIsMobileDropdownOpen(false); }}>
                                            404
                                        </NavLink>
                                    </li>
                                </ul>
                            </li>
                            <li className="nav-item mobile-btns">
                                <NavLink to="/signin" className="nav-links-btn" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Sign in
                                </NavLink>
                                <Link to="/request-demo" className="nav-links-btn btn-primary" onClick={() => { handleLinkClick(); setShowMobileMenu(false); }}>
                                    Request Demo
